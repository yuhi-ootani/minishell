...
bool	ft_array_nbr_copy(char **array_dst, char **array_src, size_t dst_start,
		size_t dst_end)
{
	size_t	dst_i;
	size_t	src_i;
	...
	{
		array_dst[dst_i] = array_src[src_i];
		dst_i++;
		src_i++;
	}
	return (true);
...
char	**append_string_arrays(char **array1, char **array2)
{
	...
	len1 = 0;
	len2 = 0;
	// 1. Count how many strings are in array1
	if (array1)
		len1 = ft_array_count_str(array1);
	// 2. Count how many strings are in array2
	if (array2)
		len2 = ft_array_count_str(array2);
	// 3. Allocate new array for all pointers + 1 for NULL terminator
	new_array = (char **)malloc(sizeof(char *) * (len1 + len2 + 1));
	if (!new_array)
		return (NULL); // Allocation failed
	// 4. Copy pointers from array1
	ft_array_nbr_copy(new_array, array1, 0, len1);
	ft_array_nbr_copy(new_array, array2, len1, len1 + len2);
...
char	**expander(char **arr)
{
	...
	while (arr[i])
	{
		splied_args = ft_split(arr[i], ' ');
		new_array = append_string_arrays(result, splied_args);
		free(result);
		result = new_array;
		i++;
	}
...
int	main(void)
{
	char	*arr[] = {"Hello", "World", " word                          split",
			" Cut   Them     ", " popo   ", NULL};
	char	**merged;
